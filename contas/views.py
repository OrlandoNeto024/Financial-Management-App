...
def ver_contas(request):
    ...
    total_contas_restantes = len(restantes)
    


    return render(request, 'ver_contas.html', {'contas_vencidas': contas_vencidas, 'contas_proximas_vencimento': contas_proximas_vencimento, 'restantes': restantes, 'total_contas_vencidas': total_contas_vencidas, 'total_contas_proximas_vencimento': total_contas_proximas_vencimento, 'total_contas_restantes': total_contas_restantes})

def pagar_conta(request, id):
    data = datetime.now()
    contas = ContaPaga(data_pagamento=data, conta_id=id)
